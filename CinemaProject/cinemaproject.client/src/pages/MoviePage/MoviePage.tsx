...
const MoviePage: React.FC = () => {
  ...
  const [movieError, setMovieError] = useState<{ is: boolean; text: string }>({
    is: true,
    text: "Завантаження...",
  });
  ...
  const sessionsScroll = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setMovie(
          await userMoviesContext.getMovieById(Number.parseInt(id || "0")),
        );
        ...
        setMovieError({
          is: true,
          text: "Помилка завантаженя інформації про фільм",
        });
        return;
      }
  ...
  const sessionBtnPress = () => {
    if (sessionsScroll.current) {
      sessionsScroll.current.scrollIntoView({
        behavior: "smooth",
      });
    }
  };
  ...
  return (
    <div className={styles.pageWrapper}>
      <section className={styles.mainInfo}>
        <div className={styles.contentContainer}>
          <div className={styles.posterSide}>
            <img
              src={`https://image.tmdb.org/t/p/w500${movie.mainInfo.posterPath}`}
          ...
          <div className={styles.detailsSide}>
            ...
            <p className={styles.metaInfo}>
              {movie.extraInfo.genres.map((g) => g.name).join(", ")} |{" "}
              {`${movie.extraInfo.runtime} хв`} |{" "}
              {new Date(movie.mainInfo.releaseDate).getFullYear()}
            </p>
            ...
            <button className={styles.buyBtn} onClick={sessionBtnPress}>
              Забронювати місце
            </button>
      ...
      {!sessions.isSuccess ? (
        <h1 className={styles.alertMessage}>{sessionsError.text}</h1>
      ) : (
        /*TODO: use a dedicated sessionItem component */
        <section className={styles.scheduleSection}>
          <h2 className={styles.sectionTitle}>Розклад сеансів</h2>
          <div className={styles.sessionsWrapper} ref={sessionsScroll}>
            <div className={styles.hallColumn}>
              <h3>ЗАЛ A</h3>
              {(sessions.data || [])
                .filter((s) => s.movieId == numberId)
                .filter((s) => s.hallId == 1)
                .map((session) => (
